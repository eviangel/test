...
def is_image_file(path):
    return None != re.search(r'\.(jpe?g|png|webp)$', path, re.IGNORECASE)

def walk_image_files(rootdir, verbose=None, write=True):
    indexfile = '%s.txt' % rootdir
    if os.path.isfile(indexfile):
        print("Found indexfile %s" % indexfile)

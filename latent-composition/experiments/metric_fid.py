...
# modified: imports
from resources.pytorch_fid.pytorch_fid.inception import InceptionV3
...
parser.add_argument('--outdir', type=str, default=None,
                    help='path to save computed fid score')
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu'):
    ...
    print(transform)
    ds = ImagesPathDataset(files, transforms=transform)
...
def _compute_statistics_of_path(path, model, batch_size, dims, device):
    if path.endswith('.npz'):
        ...
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
    elif 'resources/dataset' in path and os.path.isfile(os.path.join(path, 'metrics/fidpth/fidstats.npz')):
        # modified: to add load saved stats
        print("Found existing fidstats.npz file")
        f = np.load(os.path.join(path, 'metrics/fidpth/fidstats.npz'))
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
    else:
        path = pathlib.Path(path)
        # modified: add webp to path
        print("adding files...")
        files = list(path.glob('*.jpg')) + list(path.glob('*.png')) + list(path.glob('*.webp'))
        print("found %d files" % len(files))
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device)
        # modified: save computed stats as npz
        os.makedirs(os.path.join(path, 'metrics/fidpth'), exist_ok=True)
        np.savez(os.path.join(path, 'metrics/fidpth/fidstats.npz'), mu=m, sigma=s)
